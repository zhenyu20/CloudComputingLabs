...
int Coordinator::Working()
{
    ...
    std::string resp = de_en.getRESPArry(task);
    std::string rc = de_en.getErrorMessage();

    if (task.size() < 1)
    {
        rc = de_en.getErrorMessage();
        goto done;
    }
...
std::string Coordinator::UpdateDB(std::string resp)
{
    ...
    if (vote == true)
    {
        ...
        for (p_id id = 1; id <= _pnum; id++)
        {
            ...
            // check the state
            if (_pworkingRet[id] == KV_OK &&
                _pworkingDataRet[id] != de_en.getErrorMessage())
            {
                commit = true;
    ...
    {
        return de_en.getErrorMessage();
    }

    if (commit == true)
    {
        // push the log into the logger
        _lg.changeLogState(_TXID, LOG_COMMIT);
        _TXID++;
        return rc;
    }
    else
